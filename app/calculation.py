...
def calculate_optimal_diet(available_ingredients, targets):
    """
    ...
    if result.success:
        # Округляем до 10 грамм и фильтруем маленькие значения
        optimal_weights = np.round(result.x / 10) * 10
        ...
        for i, ing in enumerate(available_ingredients):
            ...
            if weight >= 10:  # включаем только если больше или равно 10г
                result_dict[ing.id] = {
                    'ingredient': ing,
                    'grams': weight,
                    'nutrition': {
        ...
        for key in total_nutrition:
            if targets[key] > 0:
                deviations[key] = round((total_nutrition[key] - targets[key]) / targets[key] * 100, 1)
            else:
                deviations[key] = 0
